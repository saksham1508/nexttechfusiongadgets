    ...
    return (
      <button
        ...
        className={`flex items-center justify-center border border-gray-300 rounded-lg px-4 py-2 bg-white hover:bg-gray-50 transition-colors ${
          disabled || isSigningIn ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
        }`}
        ...
        {isSigningIn ? (
          ...
          <>
            ...
            <span className="text-sm font-medium text-gray-700">
              {text === 'signin_with' ? 'Sign in with Google (Demo)' :
               text === 'signup_with' ? 'Sign up with Google (Demo)' :
               text === 'continue_with' ? 'Continue with Google (Demo)' :
               'Sign in (Demo)'}
            </span>
          </>
        )}
