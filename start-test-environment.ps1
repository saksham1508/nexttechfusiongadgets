...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot/backend'; cp .env.test .env; $env:PORT=3001; npm run dev"
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot/frontend'; cp .env.test .env; npm run start:test"
